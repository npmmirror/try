...
const config = {
  originalFile: './encryptedImage.jpg',
...
function decryptByCryptoJS(arraybuffer) {
  ...
  return (
    ...
    (wordArray = wordArray.toString(CryptoJS.enc.Base64)),
    CryptoJS.AES.decrypt(wordArray, key, {
      iv: iv,
...
// 直接输出文件
fs.writeFileSync('./decrypted-crypto-js.jpg', Buffer.from(decryptImgBuf.toString(CryptoJS.enc.Hex), 'hex'));
// 解密后，以 base64 编码并插入网页
fs.writeFileSync(
  './decrypted-base64.html',
  `<img src="data:image/jpg;base64,${decryptImgBuf.toString(CryptoJS.enc.Base64)}" />`
);

// 使用 NodeJS 的 crypto 模块解密并输出
const inStream = fs.createReadStream(config.originalFile);
const outStream = fs.createWriteStream('./decrypted-crypto.jpg');

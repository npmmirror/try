...
const logger = log4js.getLogger('crawler-baidu-news');
logger.level = 'debug';

const adapter = new FileSync('temp-news.json');
...
async function getSearchResultHtmlFromFile() {
  return fse.readFile('./temp-searchNewResult.html', { encoding: 'utf-8' });
}
...
function parseSearchResult(htmlText) {
  ...
  const $result = $('div.result-op');

  const list = $result
    .map((_, el) => {
      const $el = $(el);
      const $link = $el.find('h3 a');
      const $posterImg = $el.find('.c_photo img');
      const $abstract = $el.find('.c-span-last>span.c-font-normal');
      const $source = $el.find('.news-source>span.c-color-gray');
      const $time = $el.find('.news-source>span.c-color-gray2');

      const result = {
        href: $link.attr('href'), // href 是百度地址，需要转换
        title: $link.text(),
        titleHtml: $link.html(),
        // posterUrl: $posterImg.attr('src'), // 封面尺寸是 242x161 px，不一定有，且图片地址为百度，需要主动下载
      ...
      if (result.titleHtml) {
        result.titleHtml = result.titleHtml.replace(new RegExp('<!--/?s-text-->', 'g'), '');
      }
      if (result.abstractHtml) {
        result.abstractHtml = result.abstractHtml.replace(new RegExp('<!--/?s-text-->', 'g'), '');
      }
...
async function searchCrawler() {
  await queue.add(async () => {
    ...
    await fse.writeFile('./temp-searchNewResult.html', htmlText);
      ...
      await queue.add(async () => {
        ...
        if (item.actualUrl) {
          try {
            ...
            await fse.writeFile(path.resolve(__dirname, 'news-img', screenshot), buf);
        ...
        //     await fse.writeFile(path.resolve(__dirname, 'news-img', poster), posterBuf);
